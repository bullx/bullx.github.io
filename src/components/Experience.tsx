      ...
      <div className="section-container">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-slate-600 max-w-2xl mx-auto">
            From startup intern to Senior Software Engineer, my journey building quality software
          </p>
        </div>
        ...
        <div className="relative">
          {/* Timeline Line */}
          <div className="hidden lg:block absolute left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-primary-300 via-accent-300 to-success-300" />
            ...
            {experiences.map((exp, index) => (
              ...
              >
                ...
                <div className="hidden lg:block absolute left-1/2 -translate-x-1/2 top-8">
                  <div className={`w-4 h-4 rounded-full border-4 border-white bg-${exp.color}-600`} />
                </div>
                  ...
                  <div className="card p-6">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          <Building2 className={`w-5 h-5 text-${exp.color}-600`} />
                          <h3 className="text-xl font-bold text-slate-800">{exp.company}</h3>
                        </div>
                        <p className={`font-semibold text-${exp.color}-600`}>{exp.role}</p>
                      </div>
                    ...
                    {exp.metrics.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-4">
                        {exp.metrics.map((metric) => (
                          <span key={metric.label} className="metric-highlight text-sm">
                    ...
                    <div className="space-y-2">
                      {exp.highlights.slice(0, expandedIndex === index ? undefined : 3).map((highlight, i) => (
                        <div key={i} className="flex gap-2 text-sm text-slate-600">
                          <span className={`text-${exp.color}-600 mt-1`}>•</span>
                          <span>{highlight}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    {exp.highlights.length > 3 && (
                      <button
                        ...
                        className="flex items-center gap-1 mt-4 text-sm font-medium text-primary-600 hover:text-primary-700 transition-colors"
                      >
                        {expandedIndex === index ? (
                          <>
                            <ChevronUp size={16} />
